...
class Deliveries_view(LoginRequiredMixin,View):
    login_url = '/login/'
    def get(self,request,**kwargs):
        c = Deliveries.objects.filter(del_id_id = kwargs['mid'],inning = 1).order_by('-over','-ball')
        for i in c:
            i.over = i.over - 1
        cc = Deliveries.objects.filter(del_id_id=kwargs['mid'], inning=2).order_by('-over','-ball')
        for i in cc:
            i.over = i.over - 1
        ...
        for i in c1:
            ...
            c9 = Deliveries.objects.filter(del_id_id=i.match_id, inning=2).exclude(player_dismissed='').aggregate(
                Count('player_dismissed'))
            c10 = Deliveries.objects.filter(del_id_id=i.match_id, inning=1).order_by('-over', '-ball')
            c11 = Deliveries.objects.filter(del_id_id=i.match_id, inning=2).order_by('-over', '-ball')
            i.inn1 = c6['total_runs__sum']
            i.inn2 = c7['total_runs__sum']
            i.w1 = c8['player_dismissed__count']
            i.w2 = c9['player_dismissed__count']
            if c10[0].ball >= 6:
                i.inn1_overs = c10[0].over
            else:
                i.inn1_overs = str(c10[0].over - 1)+'.'+str(c10[0].ball)
            if c11[0].ball >= 6:
                i.inn2_overs = c11[0].over
            else:
                i.inn2_overs = str(c11[0].over - 1)+'.'+str(c11[0].ball)
...
class pointsTableView(View):
    def get(self,request,**kwargs):
        c = Matches.objects.filter(season = kwargs['year']).exclude(winner='').values('winner').annotate(Count('winner')).order_by('-winner__count')
        return render(
            request,
